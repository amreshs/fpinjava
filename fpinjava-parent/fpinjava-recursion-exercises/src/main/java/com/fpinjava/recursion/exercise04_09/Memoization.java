...
public class Memoization {
  ...
  public static Function<BigInteger, Function<BigInteger, Function<BigInteger, Function<List<BigInteger>,TailCall<List<BigInteger>>>>>> fibo_= 
          acc1 -> acc2 -> num -> lst-> (num == BigInteger.ZERO)? TailCall.res(lst) : TailCall.sus(() -> Memoization.fibo_.apply(acc2).apply(acc2.add(acc1)).apply(num.subtract(BigInteger.ONE)).apply(append(lst, acc2)));
  ...
  public static String fiboCorecursive(int number){
      ...
      List<BigInteger> lst = map(iterate(seed, fun, number+1), x -> x._1);
  ...
  */
  public static Function<List<BigInteger>, Function<String, String>> makeString_= 
        lst -> separator -> lst.isEmpty()? "" : tail(lst).isEmpty()? "" + head(lst) : foldLeft(lst, "", x -> y -> x + separator + y);
